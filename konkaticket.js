...
class Ticket {
    ...
    buyTicket() {
        ...
        let numberOfPeopleElement = document.querySelector(`.numberOfPeople[data-ticket-index="${this.index}"]`);
        let timeElement = document.querySelector(`.time[data-ticket-index="${this.index}"]`);
        let ticketNumberElement = document.querySelector(`.ticketNumber[data-ticket-index="${this.index}"]`);
    ...
    generateHTML() {
        ...
        let optionsHTML = this.options.map((option, index) =>
            `<option value="${index + 1}">${option} Person${index > 0 ? 's' : ''}</option>`
        ).join('\n');
...
function updatePrice(selectElement, pricePerPerson, ticketIndex) {
    ...
    let numberOfPeopleElement = document.querySelector(`.numberOfPeople[data-ticket-index="${ticketIndex}"]`);
    let timeElement = document.querySelector(`.time[data-ticket-index="${ticketIndex}"]`);
    let ticketNumberElement = document.querySelector(`.ticketNumber[data-ticket-index="${ticketIndex}"]`);
...
let tickets = [
    new Ticket('Day1 Pass', 150, '26th Dec', ['1', '2', '3', '4', '5', '6', '7', '8'], 0),
    new Ticket('Day2 Pass', 250, '27th Dec', ['1', '2', '3', '4', '5', '6', '7', '8'], 1),
    new Ticket('Full Event Pass', 550, '29th Dec', ['1', '2', '3', '4', '5', '6', '7', '8'], 2)
];
